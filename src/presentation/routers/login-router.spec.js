...
describe('Login Router', () => {
  test('Shold return 400 if no email is provider', async() => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
  ...
  test('Shold return 500 if no httpRequest is provider', async() => {
    const { sut } = makeSut()
    const httpResponse = await sut.route()
    expect(httpResponse.statusCode).toBe(500)
    expect(httpResponse.body).toEqual(new ServerError())
  })
  test('Shold return 500 if no httpRequest has no body', async() => {
    const { sut } = makeSut()
    const httpResponse = await sut.route({})
    expect(httpResponse.statusCode).toBe(500)
    expect(httpResponse.body).toEqual(new ServerError())
  ...
  test('Shold return 500 if no AuthUseCase has no auth method', async () => {
    const sut  = new LoginRouter({})
    const httpRequest = {
      body: {
        email: 'any_email@mail.com',
  ...
  test('Shold return 500 if no AuthUseCase throws', async () => {
